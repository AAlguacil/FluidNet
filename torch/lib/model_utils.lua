...
local function getConvLayer(mconf, ifeats, ofeats, k, up, rank, interFeats,
                            dilate)
  local conv
  if dilate == 1 or dilate == nil then
    -- We're using a standard convolution (no dilation).
    ...
    -- Otherwise we're adding a dilated convolution.
    assert(math.fmod(k, 2) == 1, 'convolution size must be odd')
    assert(up == 1, 'upsampling not supported for dilated convolutions.')
    assert((rank == 3 and mconf.is3D) or (rank == 2 and not mconf.is3D),
           'dilated convolutions must be full-rank')
    -- From torch docs:
    -- --> owidth  = floor((width + 2 * padW - dilationW * (kW-1) - 1) / dW) + 1
    -- make owdith == width and solve for padW (with dW == 1).
    local pad = dilate * (k - 1) / 2 
    assert(pad == math.floor(pad), 'Padding is not an integer value')
    -- Double check we got the math right.
    local isize = 128
    local d = 1
    local osize = math.floor((isize + 2 * pad - dilate * (k - 1) - 1) / d) + 1
    assert(isize == osize, 'Our padding math is incorrect!')
    if not mconf.is3D then
      conv = nn.SpatialDilatedConvolution(
          ifeats, ofeats, k, k, d, d, pad, pad, dilate, dilate)
    else
      conv = nn.VolumetricDilatedConvolution(
          ifeats, ofeats, k, k, k, d, d, d, pad, pad, pad, dilate, dilate,
          dilate)
    end
  end
  return conv 
...
function torch.addConv(mconf, input, ifeats, ofeats, k, up, rank, interFeats,
  ...
  if gatedConv == nil then
    gatedConv = false  -- Default is a standard convolution.
  end

  local conv = getConvLayer(mconf, ifeats, ofeats, k, up, rank, interFeats,
                            dilate)
  print('Adding convolution: ' .. conv:__tostring__())

  if gatedConv then
    -- We're going to make a gated convolution layer as described in:
    -- https://arxiv.org/pdf/1612.08083.pdf
    -- Language Modeling with Gated Convolutional Networks.
    -- Also make a conv layer for the gating function.
    print('Adding convolution gate')
    local cGate = getConvLayer(mconf, ifeats, ofeats, k, up, rank, interFeats,
                               dilate)
    return nn.CMulTable()({conv(input), cGate(input)}), conv
  else
